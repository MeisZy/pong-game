...
import turtle, os, sys, math, random, time, logzero
...
logger.info("Status: Inititialising")
...
def mainloop():
    ...
    if score_a > 9 or score_b > 9:
        logger.info("Status: Game End")
        logger.warning("Closing down, do not edit!")
        sys.exit()
...
try: 
    mainloop()
    turtle.mainloop()
    logger.error("Error: User forcibly closed program")
except:
    pass